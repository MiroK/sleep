...
def PVS_simulation(args):
    ...
    uanalytical = Expression(('-6*2*pi*f*a*cos(2*pi*f*tn)/(1+a*sin(2*pi*f*tn))*(Rpvs-x[1])/((Rpvs-Rv)*(1+a*sin(2*pi*f*tn)))*(1-(Rpvs-x[1])/(((Rpvs-Rv)*(1+a*sin(2*pi*f*tn)))))*(L-x[0])',0), degree=2, L=L, Rpvs=Rpvs,Rv=Rv, tn=0, f=fi[0], a=ai[0])

    uf_n = project(uanalytical , Wf.sub(0).collapse())
    ...
    while time < tfinal:
        ...
        #update velocity
        uanalytical.tn=time
        uf_ = project(uanalytical , Wf.sub(0).collapse())
        ...
        c_, T0= solve_adv_diff(Ct, velocity=uf_, phi=Constant(1), f=Constant(0), c_0=c_n, phi_0=Constant(1),
