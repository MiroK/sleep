...
def PVS_simulation(args):
    ...
    functionA = pi*(Rpvs**2-Rv**2)*(1+sum([a*sin(2*pi*f*tn+phi) for a,f,phi in zip(ai,fi,phii)])) # displacement
    ...
    logging.info('\n * Downstram assumption')
    logging.info('Flow from the penetrating arteriole PVS')

    logging.info('\n * Cross section area parameters')
    ai_ds=args.ai_downstream
    Rv_ds=args.Rv_downstream
    Rpvs_ds=args.Rpvs_downstream
    L_ds=args.L_downstream


    V0_ds=(pi*Rpvs_ds**2-pi*Rv_ds**2)*L_ds

    functionV_ds = V0_ds*sum([a*sin(2*pi*f*tn+phi) for a,f,phi in zip(ai_ds,fi,phii)]) # V=A(t)*L=A0*L*(1+asin)
    functionU_ds = -sympy.diff(functionV_ds,tn)/functionA # dV/dt  ## Il faudrait donner une forme parabolique avec la fonction analytique ici
    # minus sign , as the arteriole PVS shrink the inflow velocity in the pial PVS is positive
    V_bc=sympy.printing.ccode(functionU_ds)

    vf_left = Expression((V_bc,'0' ), tn = 0, degree=2)   # inflow due to the penetrating arteriole contraction 

    logging.info('ai downstream pvs (dimensionless): '+'%e '*len(ai_ds)%tuple(ai_ds))
    logging.info('R vessel downstream pvs : '+'%e '%Rv_ds)
    logging.info('R pvs downstream pvs : '+'%e '%Rpvs_ds)
    logging.info('L vessel downstream pvs : '+'%e '%L_ds)

    logging.info('\n * Fluid')
    logging.info('Left : pownstream flow')
    ...
    if lateral_bc=='resistance' :
        ...
        bcs_fluid = {'velocity': [(facet_lookup['y_min'],vf_bottom),
                                ...
                                (facet_lookup['x_min'], vf_left)],
                    'traction': [],  
                    'pressure': [(facet_lookup['x_max'], Rpressure)]}
    else :
        bcs_fluid = {'velocity': [(facet_lookup['y_min'],vf_bottom),
                                (facet_lookup['y_max'], Constant((0,0))),
                                (facet_lookup['x_min'], vf_left)],
                    'traction': [],  
                    'pressure': [(facet_lookup['x_max'], Constant(0))]}
    ...
    driving_expressions = (uale_bottom,vf_bottom,vf_left)
...
if __name__ == '__main__':
    ...
    my_parser.add_argument('-ai_downstream',
                        type=float,
                        nargs='+',
                        default=[0.04],
                        help='List of ai for the downstream PVS')

    my_parser.add_argument('-Rv_downstream',
                        type=float,
                        default=8e-4,
                        help='Radius vessel downstream PVS')

    my_parser.add_argument('-Rpvs_downstream',
                        type=float,
                        default=10.5e-4,
                        help='Astrocytes endfeet vessel downstream PVS')

    my_parser.add_argument('-L_downstream',
                        type=float,
                        default=200e-4,
                        help='length vessel downstream PVS')
