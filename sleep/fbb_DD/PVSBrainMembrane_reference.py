...
def PVSbrain_simulation(args):
    ...
    #pvd files
    etaf_out, uf_out, pf_out= File(outputfolder+'fields'+'/etaf.pvd'), File(outputfolder+'fields'+'/uf.pvd'), File(outputfolder+'fields'+'/pf.pvd') 
    etas_out, qs_out, ps_out , = File(outputfolder+'fields'+'/etas.pvd'), File(outputfolder+'fields'+'/us.pvd'), File(outputfolder+'fields'+'/ps.pvd') 
    ...
    membrane_thickness=args.thickness_membrane
    Rmem =Rpvs+ membrane_thickness# centimeters
    ...
    logging.info('Astrocyte endfeet thickness : %e cm'%membrane_thickness)
    ...
    logging.info('N radial PVS: %e'%Nr_pvs)
    logging.info('N radial Biot: %e'%Nr_biot)
    logging.info('N radial Membrane: %e'%Nr_membrane)
    ...
    permeabilitymem=args.biot_permeability_membrane
    conductivitymem=permeabilitymem/mu
    Emem=args.biot_youngmodulus_membrane
    poissonmem=args.biot_poisson_membrane
    alphamem=args.biot_biotcoefficient_membrane
    ...
    ## The tracer is solver in the full domain, so it has three subdomains
    # 2 for tissue
    # 1 for membrane
    # 0 for fluid

    tracer_parameters={'kappa_0': D, 'kappa_1': D*tortuositymem,'kappa_2':D*tortuosity, 'dt':dt_advdiff, 'nsteps':1}
    ...
    if meshing=='regular' :
        ...
        #Creation of a uniform mesh of radias length Nvessel + Nmembrane + Nbiot

        Nr=Nr_pvs+Nr_membrane+Nr_biot
        mesh= RectangleMesh(Point(0,0), Point(L, Nr), Nl, Nr)
        ...
        def deform_mesh(x, y):
            transform_fluid=Rv + (Rpvs-Rv)*y/Nr_pvs
            transform_membrane=Rpvs + (Rmem-Rpvs)*((y-Nr_pvs)/(Nr_membrane))
            transform_brain=Rmem + (Rbrain-Rmem)*((y-Nr_pvs-Nr_membrane)/(Nr_biot))**s_biot
            yp=transform_brain
            Imembrane=np.where(y <= (Nr_pvs+Nr_membrane))[0]
            yp[Imembrane]=transform_membrane[Imembrane]
            Ivessel=np.where(y <= Nr_pvs)[0]
            yp[Ivessel]=transform_fluid[Ivessel]
        ...
        e = factory.addPoint(L,Rmem,0)
        f = factory.addPoint(0,Rmem,0)
        g = factory.addPoint(L,Rbrain,0)
        h = factory.addPoint(0,Rbrain,0)

        fluid_lines = [factory.addLine(*p) for p in ((d,a),(a,b), (b, c),(c,d) )]
        interface1=[fluid_lines.pop()]
        named_lines = dict(zip(('pvs_left','bottom', 'pvs_right'), fluid_lines))

        membrane_right=[factory.addLine(*p) for p in ((c,e),(e,f) )]
        interface2=[membrane_right.pop()]
        named_lines.update( dict(zip(('mem_right'), membrane_right)))

        membrane_left=[factory.addLine(*p) for p in ((f,d),(d,c))]
        membrane_left.pop()
        named_lines.update( dict(zip(( 'mem_left'), membrane_left)))


        solid_lines = [factory.addLine(*p) for p in ((e, g), (g,h), (h,f))]
        named_lines.update(dict(zip(( 'brain_right', 'brain_top', 'brain_left'), solid_lines)))

        named_lines.update(dict(zip(( 'interface1'), interface1)))
        named_lines.update(dict(zip(( 'interface2'), interface2)))

        fluid_loop = factory.addCurveLoop(fluid_lines+interface1)
        fluid = factory.addPlaneSurface([fluid_loop])

        mem_loop = factory.addCurveLoop(membrane_left+interface1+membrane_right+interface2)
        membrane = factory.addPlaneSurface([mem_loop])

        solid_loop = factory.addCurveLoop(interface2+solid_lines)
        solid = factory.addPlaneSurface([solid_loop])


        factory.synchronize()

        tags = {'cell': {'F': 1, 'S': 2,'M':3},
            'facet': {}}

        model.addPhysicalGroup(2, [fluid], 1)
        model.addPhysicalGroup(2, [solid], 2)
        model.addPhysicalGroup(2, [membrane], 3)
        ...
        field.setNumber(fid, 'YMin', Rpvs)
        field.setNumber(fid, 'YMax', Rmem)
        field.setNumber(fid, 'VIn', membrane_cell_size)
        ...
        field.setNumber(fid, 'VIn', pvs_cell_size)
        field.setNumber(fid, 'VOut', DR*50 )
        field.setNumber(fid, 'Thickness', (Rpvs-Rv)/4)
    ...
    class Omega_fluid(SubDomain):
        def inside(self, x, on_boundary):
            return x[1] < Rpvs + tol
    class Omega_solid(SubDomain):
        def inside(self, x, on_boundary):
            return x[1] > Rpvs - tol


    class Omega_membrane(SubDomain):
        def inside(self, x, on_boundary):
            return (x[1] > Rpvs - tol)&(x[1] < Rmem + tol)

    class Omega_tissue(SubDomain):
        def inside(self, x, on_boundary):
            return x[1] > Rmem - tol

    subdomains = MeshFunction("size_t", mesh, mesh.topology().dim(),0)
    subsubdomains = MeshFunction("size_t", mesh, mesh.topology().dim(),0)
    

    subdomain_fluid = Omega_fluid()
    subdomain_solid = Omega_solid()

    subdomain_fluid.mark(subdomains, 0)
    subdomain_solid.mark(subdomains, 1)


    mesh_f = EmbeddedMesh(subdomains, 0)
    mesh_s = EmbeddedMesh(subdomains, 1)

    # define two subdomains for different materials in the solid

    materials = MeshFunction("size_t", mesh_s, mesh_s.topology().dim(),0)

    subdomain_membrane = Omega_membrane()
    subdomain_tissue = Omega_tissue()

    subdomain_membrane.mark(materials, 0)
    subdomain_tissue.mark(materials, 1)

    subdomain_fluid.mark(subsubdomains, 0)
    subdomain_membrane.mark(subsubdomains, 1)
    subdomain_tissue.mark(subsubdomains, 2)
    ...
    # Pick system solver
    solid_parameters['solver'] = 'direct'

    # delta t
    solid_parameters['dt']=dt_solid 

    # define the domain specific parameters for the tracer
    # NOTE: Here we do P0 projection
    dx = Measure('dx', domain=mesh, subdomain_data=subsubdomains)
    ...
    membrane_coef = tracer_parameters.pop('%s_1' % coef)
    solid_coef = tracer_parameters.pop('%s_2' % coef)

    form = ((1/CellVolume(mesh))*fluid_coef*q*dx(0) +
                (1/CellVolume(mesh))*membrane_coef*q*dx(1)+
                (1/CellVolume(mesh))*solid_coef*q*dx(2))
    ...
    form = ((1/CellVolume(mesh))*Constant(0)*q*dx(0) +
                ...
                (1/CellVolume(mesh))*Constant(1)*q*dx(2))
                
    mask_solid=Function(CoefSpace, assemble(form))

    form = ((1/CellVolume(mesh))*Constant(0)*q*dx(0) +
                (1/CellVolume(mesh))*Constant(1)*q*dx(1)+
                (1/CellVolume(mesh))*Constant(0)*q*dx(2))
                
    mask_membrane=Function(CoefSpace, assemble(form))

    form = ((1/CellVolume(mesh))*Constant(0)*q*dx(0) +
                (1/CellVolume(mesh))*Constant(0)*q*dx(1)+
                (1/CellVolume(mesh))*Constant(1)*q*dx(2))
                
    mask_tissue=Function(CoefSpace, assemble(form))
    ...
    # U for ALE at the vessel boundary (from reference frame)
    functionUALE=Rv*(sum([a*sin(2*pi*f*tnp1+phi) for a,f,phi in zip(ai,fi,phii)]))
    ...
    porositys_n=project(Constant(porosity_0)*mask_tissue+Constant(porosity_0mem)*mask_membrane,FS_porositys)
    ...
    if abs(tshift):
        ...
        transfer_into(eta_interface, Constant((0,0)), boundaries,tol=1E-10)
...
if __name__ == '__main__':
    ...
    my_parser.add_argument('-hmem','--thickness_membrane',
                        type=float,
                        default=1e-4,
                        help='Thickness of the astrocyte endfeet layer')
    ...
    my_parser.add_argument('-nrmem','--N_radial_membrane',
                        type=int,
                        default=3,
                        help='Number of cells in radial direction in the membrane')
    ...
    my_parser.add_argument('-Emem','--biot_youngmodulus_membrane',
                        type=float,
                        default=10e4,
                        help='Young modulus in the membrane (dyn/cm2)')

    my_parser.add_argument('-numem','--biot_poisson_membrane',
                        type=float,
                        default=0.45,
                        help='Poisson coefficient in the membrane')

    my_parser.add_argument('-alphamem','--biot_biotcoefficient_membrane',
                        type=float,
                        default=1,
                        help='Biot coefficient membrane')
