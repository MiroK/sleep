...
def solve_solid(W, f1, f2, eta_0, p_0, bdries, bcs, parameters, nitsche_penalty):
    ...
    for tag, (vel_data, stress_data) in nitsche_t_bcs:
        ...
        a += (
              ...
              -inner(wedge(dot(sym(grad(eta)), n), n), wedge(phi, n))*ds(tag)
              -inner(wedge(dot(sym(grad(phi)), n), n), wedge(eta, n))*ds(tag)
              #-inner(dot((grad(wedge(eta,n))),n), wedge(phi, n))*ds(tag)
              #-inner(dot((grad(wedge(phi,n))),n), wedge(eta, n))*ds(tag)
        ...
        L += (
            ...
            -inner(wedge(dot(sym(grad(phi)), n), n), vel_data)*ds(tag)
            #-inner(dot((grad(wedge(phi,n))),n),  vel_data)*ds(tag)
            ...
            #+stress_data*inner(n,phi) * ds(tags[tag]) 
        )
...
if __name__ == '__main__':
    ...
    if round_top:
        template = '/workspaces/sleep/sleep/mesh/test/round_domain_{}.h5'
    else:
        template = '/workspaces/sleep/sleep/mesh/test/square_domain_{}.h5'        
