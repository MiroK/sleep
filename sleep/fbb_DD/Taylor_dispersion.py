...
def slice_integrate(x,f,ymin,ymax,N=10) :
    ...
    values=line_sample(vertical_line, f)
    r=np.linspace(ymin,ymax,N)
    integral=np.trapz(values,r)/(ymax-ymin) #cylindrical coordinate
    return integral
...
def PVS_simulation(args):
    ...
    logging.info(title1("Simulation of the adv diffusion in an imposed fluid velocity field"))
    ...
    usteady = Expression(('6*U*(Rpvs-x[1])/(Rpvs-Rv)*(1-(Rpvs-x[1])/(Rpvs-Rv))',0), degree=2, U=Umean, Rpvs=Rpvs,Rv=Rv)
    ...
    usteady = Expression(('-6*2*pi*f*a*cos(2*pi*f*tn)/(1+a*sin(2*pi*f*tn))*(Rpvs-x[1])/(Rpvs-Rv)*(1-(Rpvs-x[1])/(Rpvs-Rv))*(x[0])',0), degree=2, L=L, U=Umean, Rpvs=Rpvs,Rv=Rv, tn=0, f=fi[0], a=ai[0])
    ...
    while time < tfinal:
        ...
        c_, T0= solve_adv_diff(Ct, velocity=uf_n, phi=Constant(1), f=Constant(0), c_0=c_n, phi_0=Constant(1),
