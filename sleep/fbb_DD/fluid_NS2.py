...
def solve_fluid(W, f, u_n, p_n, bdries, bcs, parameters):
    ...
    U = 0.5*(u + u_n)
    F1 = (1./k)*inner(u - u_n, v)*r*dx + inner(dot(grad(u_n), u_n), v)*r*dx\
         + mu*inner(grad(U), grad(v))*r*dx + inner(grad(p_n), v)*r*dx\
         + mu * u[1] * v[1] / r * dx\
         - inner(f, v)*r*dx
    a1, L1 = system(F1)

    # Projection, solve to p1
    F2 = inner(grad(p - p_n), grad(q))*r*dx + (1./k)*q* (( u_[1]).dx(1) + u_[0].dx(0))*r*dx
    a2, L2 = system(F2)

    # Finalize, solve to u1
    F3 = (1./k)*inner(u - u_, v)*r*dx + inner(grad(p_ - p_n), v)*r*dx 
    a3, L3 = system(F3)
