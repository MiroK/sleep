...
def PVS_simulation(args):
    ...
    logging.info(title1("Simulation of the PVS flow and tracer transport in a peristaltic flow"))
    ...
    ##Here I put several wavelengths
    ...
    class PeriodicBoundary(SubDomain):
        ...
        def inside(self, x, on_boundary):
            return bool(x[0] < xmin+tol and on_boundary)
    ...
    Wf = FunctionSpace(mesh_f, Wf_elm)#, constrained_domain=pbc)
    ...
    vf_bottom = Expression(('0','h0*phi*w*sin(w*tn-k*x[0])' ),h0=(Rpvs-Rv),phi=ai[0], w=2*pi*fi[0],k=2*pi/L, tn = 0, degree=2)   # no slip no gap condition at vessel wall 
    uale_bottom =  Expression(('0','h0*phi*(cos(w*tn-k*x[0]) - cos(w*tnp1-k*x[0]))' ),h0=(Rpvs-Rv),phi=ai[0], w=2*pi*fi[0],k=2*pi/L, tn = 0, tnp1=0, degree=2)  # displacement for ALE at vessel wall 
    ...
    Vinflow= Expression(('0','w*phi/k*cos(w*tn)*(1-phi*cos(w*tn))*6*(Rpvs-x[1])/(h0*(1+phi*cos(w*tn)))*(1-(Rpvs-x[1])/(h0*(1+phi*cos(w*tn))))' ),Rpvs=Rpvs,h0=(Rpvs-Rv),phi=ai[0], w=2*pi*fi[0],k=2*pi/L, tn = 0, degree=2) 
    ...
    driving_expressions = (uale_bottom,vf_bottom,Vinflow)
    ...
    while time < tfinal:
        ...
        #set periodic pressure by imposing left pressure from last time step to the right pressure
        #z, r = SpatialCoordinate(mesh_f)
        #ds = Measure('ds', domain=mesh_f, subdomain_data=fluid_bdries)
        #n = FacetNormal(mesh_f)
        #Pmean=assemble(pf_n*ds(facet_lookup['x_min']))
        #S=assemble(Constant(1)*ds(facet_lookup['x_min']))
        #setattr(Pout, 'pvalue', Pmean/S)
...
if __name__ == '__main__':
    ...
    my_parser.add_argument('-kale','--ale_parameter',
                        type=float,
                        default=1,
                        help='ALE parameter')
