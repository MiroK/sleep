...
def PVS_simulation(args):
    ...
    functionR = Rv*(1+sum([a*sin(2*pi*f*tn+phi) for a,f,phi in zip(ai,fi,phii)])) # displacement bottom plate
    ...
    logging.info("\n * Tracer")
    logging.info("Concentration : zero")
    ...
    #Let define an arbitrary source term

    f_1 = Expression('pow(x[0]-l*0.9, 2) + pow(x[1]-(Rv+Rpvs)/2, 2) < (Rv+Rpvs)*(Rv+Rpvs)/4 ? 0.1 : 0', Rv=Rv,Rpvs=Rpvs,l=L, degree=1)
    ...
    while time < tfinal+tshift:
        ...
        c_, T0= solve_adv_diff(Ct, velocity=uf_-eta_f/Constant(dt), phi=Constant(1), f=f_1, c_0=c_n, phi_0=Constant(1),
